...
export default function FormLoginDokter() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
     ...
     const expectedRole = 3; // Misalnya, role 2 untuk User

     await login(credentials, expectedRole); // Panggil fungsi login dari AuthContext
     const redirectTo = location.state?.from?.pathname || "/homedokter";
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-8">
      ...
      <div className="flex flex-col gap-4">
        ...
        <Button className="w-full text-primary font-bold" variant="outline" size="lg" onClick={() => navigate("/register-dokter")}>
