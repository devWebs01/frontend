...
const ReviewSection = () => {
  return (
    <motion.section
      className="flex overflow-hidden flex-col justify-center self-stretch px-36 py-36 bg-[linear-gradient(100deg,rgba(127,195,176,0.15_7.05%,rgba(78,172,146,0.10)_50.64%))] max-md:px-5 max-md:py-24 bg-[#D5EFE9]"
      initial={{ opacity: 0 }}
    ...
    >
      <div className="flex flex-wrap gap-10 items-center w-full max-md:max-w-full">
        ...
        {reviewData.map((review, index) => (
          <motion.div
            key={index}
            className="flex flex-col flex-1 shrink self-stretch my-auto rounded-none basis-0 min-w-[332px]"
            initial={{ opacity: 0, y: 50 }}
            whileInView={{ opacity: 1, y: 0 }}
            transition={{ duration: 0.8, delay: index * 0.3 }} // Delay meningkat setiap item untuk efek bertahap
            viewport={{ once: false }}
          >
            <ReviewCard {...review} />
          </motion.div>
